...
def dbconnection():
    conn = mysql.connector.connect(
            host = 'verifacedb-facedb.j.aivencloud.com',
            user = 'avnadmin',
            database = 'veriface',
            password = 'AVNS_HnUjyILMXi-IlvR3Iy2',
            port = '14284'
        )
    return conn
...
def get_atendance(event_id):
    list_hadir = []
    nama_hadir = []
    conn = dbconnection()
    myconn=conn.cursor()
    query = f"SELECT k.id_peserta, p.nama FROM kehadiran AS k JOIN peserta AS p ON k.id_peserta = p.id_peserta WHERE k.tanggal_kehadiran = '{datetime.now().strftime('%Y-%m-%d')}' AND id_event = '{event_id}'"
...
def updateHadirInfo(event_id):
    list_hadir, nama_hadir = get_atendance(event_id)
...
def tambah(id, nama, event_id):
    global frame
    userid = str(id)
    userimagefolder = 'static/faces/' + event_id + "/" + userid + "_" + nama
    i = 1
    ...
    while True:
        ...
        if not os.path.isdir("static/faces/" + event_id):
            os.makedirs("static/faces/" + event_id)   
        if os.listdir("static/faces/" + event_id):
            res = DeepFace.find(frame, db_path="static/faces/" + event_id , enforce_detection=False, model_name="Facenet512")
            if len(res[0]['identity'])>0 and i == 1:
                socketio.emit('wajahTerdaftar')
        else:        
            conn = dbconnection()
            myconn = conn.cursor()
            ...
            if len(faces) == 1:  # Ensure only one face is detected
                for (x, y, w, h) in faces:
                    ...
                    face_count += 1
                    if face_count == 50:
                        DeepFace.find(frame, db_path="static/faces/" + event_id, enforce_detection=False, model_name="Facenet512")
                        # camera.release()
...
def cek(event_id):
    ...
    while True:
        ...
        if len(res[0]['identity'])>0:
            name = res[0]['identity'][0].split('/')[2].split("\\")[1].split("_")[1]
            id = res[0]['identity'][0].split('/')[2].split("\\")[1].split("_")[0]
            ...
            if int(id) not in list_hadir:
                add_attendance(id)
                # myconn.execute(query)
                list_hadir.append(int(id))
                # list_hadir = [item[0] for item in myconn.fetchall()]
                verif = 0
        else:
            verif = 0
            faces = extract_faces(frame)
